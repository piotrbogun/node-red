...
Another ticket that seemed like a quick fix as I was just poking around Node-RED. Not specifically done for the take home assignment. Including it since it's just something I did.
...
When filtering the tree list, items are simply hidden in the DOM, but the list of items itself is not filtered, or any type of filter flagging. This is what I intentionally left out when deciding on approach and just chose as a step 1 to just check if the item is hidden when doing arrow key navigation.

### (2) If you used AI as a tool while building this, how and why did you decide to use it?

I used AI to quickly find that `EditableList` is responsible for hiding filtered items with `.hide()`, which helped me understand the current logic. Used it for speed.
...
The goal was to stick to the main focus of moving to NPM workspaces.
